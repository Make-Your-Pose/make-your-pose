  ...
  useEffect(() => {
    const drawMask = () => {
      ...
      if (canvas) {
        ...
        console.log('Canvas available, segmentationMask:', !!segmentationMask);
        ...
        if (ctx && segmentationMask) {
          ...
          console.log('Canvas resized to match mask dimensions');
          ...
          try {
            ...
            if (segmentationMask.hasFloat32Array()) {
              console.log('Float32Array 형식의 마스크 데이터 사용');
              maskBuffer = segmentationMask.getAsFloat32Array();
              maskType = 'float32';
              console.log('Float32Array retrieved, length:', maskBuffer.length);
            } else if (segmentationMask.hasUint8Array()) {
              console.log('Uint8Array 형식의 마스크 데이터 사용');
              maskBuffer = segmentationMask.getAsUint8Array();
              maskType = 'uint8';
              console.log('Uint8Array retrieved, length:', maskBuffer.length);
            } else if (segmentationMask.hasWebGLTexture()) {
              console.log('WebGLTexture에서 Float32Array로 변환 시도 중...');
              maskBuffer = segmentationMask.getAsFloat32Array();
              maskType = 'float32';
              console.log(
                'WebGLTexture에서 변환 완료, length:',
                maskBuffer.length,
              );
            } else {
            ...
            console.log('픽셀 데이터 변환 완료');

            // --- 6. 캔버스에 그리기 ---
            console.log('캔버스에 마스크 데이터 그리기...');
            ctx.putImageData(imageData, 0, 0);
            console.log('마스크 렌더링 완료');
          } catch (error) {
            console.error('마스크 처리 또는 렌더링 중 오류 발생:', error);
          } finally {
            console.log('마스크 처리 완료');
          }
        }
      }
